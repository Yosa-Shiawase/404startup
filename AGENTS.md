...
## Waiting room: archive (v12)
library/archive/ is a WAITING ROOM (type:"waiting" in manifest, tags ambient/
mood). paperfall.html = ambient page (self-contained); archive_page.html =
hand-written description page (no PAIRS line, never regenerated by
build_library.sh). Art via window.N404_ART.archive post-IIFE form.
Waiting rooms have NO score and NO lives - mood pages only. The library
WAITING chip activates from this entry. More waiting rooms follow the same
pattern: own folder, manifest entry type:"waiting", art entry, hand-made
_page.html, sw PRECACHE both files, bump.
