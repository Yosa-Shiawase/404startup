/*! 404STARTUP (c) 2025 Yosa-Shiawase. All rights reserved. No copying, reuse or hosting without permission. Custom builds: instagram.com/yosashiawase */
/* ============================================================
   N404 CORE — readable engine source.
   Ship via: terser n404-core.src.js -c -m -o ../n404-core.js
   Public contract (frozen): window.N404 = {mount, Games, INFO}
   Games ids: dunes,orbit,terminal,sonar,vapor,matrix,cab,deep
   Score keys: n404_best_<id>   Mute key: n404_mute
   ============================================================ */
...
/* ---------- WebAudio synth ---------- */
const AudioFX={
  ctx:null,master:null,muted:LS.get('n404_mute','0')==='1',
  ensure(){
    if(!this.ctx){
  ...
  tone(f0,f1,dur,type,vol,at){
    ...
    type=type||'square'; vol=vol==null?1:vol; at=at||0;
...
/* ---------- game metadata (exact strings as shipped) ---------- */
const INFO={
  dunes:{name:'DUNES FOX RUN',meta:'#F2E2C4',
    hintC:'TAP TO RUN & JUMP \u00B7 SWIPE \u2193 TO DUCK',
    hintK:'SPACE TO RUN \u00B7 \u2191 JUMP \u00B7 \u2193 DUCK',
    over:'LOST IN THE DUNES'},
  orbit:{name:'ORBIT DRIFTER',meta:'#0A0E1B',
    hintC:'TAP TO LAUNCH \u00B7 DRAG TO STEER',
    hintK:'SPACE TO LAUNCH \u00B7 \u2191 \u2193 OR HOLD TO STEER',
    over:'SHIP LOST IN THE VOID'},
  terminal:{name:'TERMINAL SNAKE',meta:'#050805',
    hintC:'TAP TO START \u00B7 SWIPE OR USE THE PAD',
    hintK:'SPACE TO START \u00B7 ARROWS / WASD',
    over:'PROCESS TERMINATED'},
  sonar:{name:'SONAR CONTACT',meta:'#04101A',
    hintC:'TAP TO SCAN \u00B7 TAP THE PINGS',
    hintK:'SPACE TO SCAN \u00B7 CLICK THE PINGS',
    over:'SIGNAL FADED'},
  vapor:{name:'SUNSET DRIFT',meta:'#160B33',
    hintC:'TAP TO DRIVE \u00B7 TAP LEFT / RIGHT FOR LANES',
    hintK:'SPACE TO DRIVE \u00B7 \u2190 \u2192 CHANGE LANES',
    over:'WIPEOUT'},
  matrix:{name:'WHITE RABBIT',meta:'#000306',
    hintC:'TAP TO JACK IN \u00B7 TAP LEFT / RIGHT TO HOP',
    hintK:'SPACE TO JACK IN \u00B7 \u2190 \u2192 TO HOP',
    over:'CONNECTION LOST'},
  cab:{name:'ARCADE CABINET',meta:'#0B0B12',
    hintC:'TAP TO INSERT COIN \u00B7 HOLD & DRAG PADDLE',
    hintK:'SPACE TO INSERT COIN \u00B7 HOLD ARROWS OR DRAG',
    over:'GAME OVER \u2014 INSERT COIN'},
...
function showOver(label,score,best,nb){
  if(!game||game.mode!=='dead')return;
  els.overL.textContent=label;
  els.overS.textContent='SCORE '+fmt(score)+' \u2014 BEST '+fmt(best);
  els.overN.classList.toggle('show',!!nb);
...
/* ghost-guard: restart only once panel is shown (or 1.2s fallback) */
function canRestart(){
  const shown=els.over.classList.contains('show');
  return shown||performance.now()-deadAt>1200;
}

/* ---------- viewport sync (guarded; watchdog re-runs are no-ops) ---------- */
...
/* ---------- single-game mount (no switcher by design) ---------- */
function mount(id,opts){
  opts=opts||{};
  gameId=id;game=Games[id];if(!game)throw new Error('no game: '+id);
  if(!document.getElementById('n404css')){
    const st=document.createElement('style');st.id='n404css';st.textContent=CSS;
  ...
  els.over.addEventListener('pointerdown',e=>{
    e.stopPropagation();
    if(game.mode==='dead'&&canRestart())game.restart();
  });
  ...
  function renderMute(){
    document.getElementById('icoOn').style.display=AudioFX.muted?'none':'';
    document.getElementById('icoOff').style.display=AudioFX.muted?'':'none';
  }
  snd.addEventListener('click',()=>{
    AudioFX.ensure();AudioFX.muted=!AudioFX.muted;
    LS.set('n404_mute',AudioFX.muted?'1':'0');renderMute();
  });
  renderMute();
...
/* ============================================================
   GAME: dunes — fennec-fox endless runner, day/night cycle
   ============================================================ */
Games.dunes=(function(){
  ...
  function die(){
    ...
    setBest(mx);
    setTimeout(()=>showOver(INFO.dunes.over,score,mx,nb),500);
  }
  function spawnObs(){
  ...
  function mixP(A,B,k){
    ...
    ['ground','top','duneFar','duneNear','ink','accent','cloud'].forEach(key=>o[key]=mix(A[key],B[key],k));
    return o;
  }
  ...
  function drawFox(gc,x,footY,cell,frame,ink,accent,eyeBg,dead,blink){
    ...
    for(let r=0;r<11;r++)for(let c=0;c<16;c++)
      if(spr[r]&&spr[r][c]==='#')gc.fillRect(ox+c*cell,oy+r*cell,Math.ceil(cell),Math.ceil(cell));
...
/* ============================================================
   GAME: orbit — debris-field drifter, pointer or key steering
   ============================================================ */
Games.orbit=(function(){
  ...
  function die(){
    ...
    setBest(mx);
    setTimeout(()=>showOver(INFO.orbit.over,score,mx,nb),750);
  }
  function spawnAst(){
...
/* ============================================================
   GAME: terminal — phosphor CRT snake, board auto-fits
   ============================================================ */
Games.terminal=(function(){
  ...
  function die(){
    ...
    setBest(mx);
    setTimeout(()=>showOver(INFO.terminal.over,score,mx,nb),550);
  }
  ...
  g.enter=()=>{layout();resetDemo();g.mode='ready';showHint(COARSE.matches?INFO.terminal.hintC:INFO.terminal.hintK);hideOver();};
  g.start=start;
  ...
  g.draw=gc=>{
    ...
    const tk=9;
    [[fx,fy,1,1],[fx+fw,fy,-1,1],[fx,fy+fh,1,-1],[fx+fw,fy+fh,-1,-1]].forEach(c=>{
...
/* ============================================================
   GAME: matrix — code-rain glyph catch, dodge the sentinels
   ============================================================ */
Games.matrix=(function(){
  ...
  function layout(){
    cw=clamp(W/18,22,46);cols=Math.max(9,Math.floor(W/cw));cw=W/cols;
  ...
  function reset(){
    g.mode='ready';items=[];lives=3;score=0;setScore(0);px=Math.floor(cols/2);spawnT=.9;flash=0;
    showHint(COARSE.matches?INFO.matrix.hintC:INFO.matrix.hintK);hideOver();
  }
  function start(){
    if(g.mode!=='ready')return;
    g.mode='playing';items=[];hideHint();hideOver();AudioFX.tone(200,700,.25,'square',.3);
  }
  function die(){
    ...
    if(nb)LS.set(bestKey('matrix'),score);
    setBest(mx);
    setTimeout(()=>showOver(INFO.matrix.over,score,mx,nb),500);
  }
  function mv(d){
    if(g.mode!=='playing')return;
    const n=clamp(px+d,0,cols-1);
  ...
  g.restart=()=>{reset();start();};
  g.resize=layout;
  g.key=k=>{if(k==='ArrowLeft'||k==='a'||k==='A')mv(-1);else if(k==='ArrowRight'||k==='d'||k==='D')mv(1);};
  g.release=()=>{};
  g.pointer=(e,type)=>{
  ...
  g.update=dt=>{
    ...
    if(g.mode==='playing'){
      spawnT-=dt;
      if(spawnT<=0){
        spawnT=rand(.45,.85)*clamp(1.25-score/700,.5,1.25);
      ...
      for(const it of items){
        it.y+=it.v*dt;
        if(it.y>catchY()&&!it.done){
          it.done=true;
          if(it.col===px){
            if(it.typ==='g'){score+=10;setScore(score,true);AudioFX.eat();}
            else{
              lives--;flash=1;AudioFX.tone(160,50,.35,'sawtooth',.5);
  ...
  g.draw=gc=>{
    ...
    for(let i=0;i<cols;i++){
      const h=heads[i];
      for(let j=0;j<h.len;j++){
        const y=h.y-j*cw;if(y<-cw||y>H+cw)continue;
    ...
    for(const it of items){
      const x=it.col*cw+cw/2;
      gc.font=fs+'px '+MONO;
      if(it.typ==='g'){
        gc.fillStyle=C.head;gc.shadowColor=C.head;gc.shadowBlur=8;gc.fillText(it.ch,x,it.y);gc.shadowBlur=0;
      }else{
        gc.fillStyle=C.bad;gc.shadowColor=C.bad;gc.shadowBlur=10;gc.fillText(it.ch,x,it.y);gc.shadowBlur=0;
    ...
    for(let i=0;i<3;i++){
      gc.beginPath();gc.arc(20+i*22,H-20,5,0,TAU);
...
/* ============================================================
   GAME: cab — breakout; relaunch on tap/space after every ball
   ============================================================ */
Games.cab=(function(){
  const g={mode:'ready'};
  const RC=['#FF5F56','#FFBD2E','#27C93F','#01CDCD','#FF71CE','#FFB454'];
  const C={bg:'#0B0B12',frame:'#2A2A3A',txt:'#FFE8C9',ball:'#FFFFFF'};
  let t=0,U=1,score=0,lives=3,level=1,bricks=[],
      ball={x:0,y:0,vx:0,vy:-1,stuck:true},pad={x:0,w:0},
      F={x:0,y:0,w:0,h:0},pHeld=false,tx=0,shake=0;
  ...
  function layout(){
    U=clamp(Math.min(H/560,W/620),.55,1.4);
  ...
  function launch(){
    ...
    ball.stuck=false;hideHint();
    const a=rand(-.5,.5),sp=(300+level*30)*U;
    ball.vx=Math.sin(a)*sp;ball.vy=-Math.cos(a)*sp;
    AudioFX.tone(500,760,.08,'square',.3);
  }
  function reset(){
    g.mode='ready';score=0;lives=3;level=1;setScore(0);layout();
    showHint(COARSE.matches?INFO.cab.hintC:INFO.cab.hintK);hideOver();
  }
  function start(){
    if(g.mode!=='ready')return;
    g.mode='playing';hideHint();hideOver();launch();
  }
  function die(){
    g.mode='dead';deadAt=performance.now();AudioFX.die();hideHint();
    const best=+LS.get(bestKey('cab'),0),nb=score>best,mx=Math.max(best,score);
    if(nb)LS.set(bestKey('cab'),score);
    setBest(mx);
    setTimeout(()=>showOver(INFO.cab.over,score,mx,nb),500);
  }
  g.enter=()=>{layout();reset();};
  g.start=start;
  g.restart=()=>{reset();start();};
  g.resize=layout;
  g.key=k=>{if((k===' '||k==='Enter')&&ball.stuck&&g.mode==='playing')launch();};
  g.release=()=>{};
  g.pointer=(e,type)=>{
  ...
  g.update=dt=>{
    ...
    for(let s=0;s<steps;s++){
      ...
      if(ball.y>F.y+F.h+30){
        lives--;AudioFX.tone(200,60,.3,'sawtooth',.5);
        if(lives<=0){die();return;}
        stickBall();
        showHint(COARSE.matches?'TAP TO LAUNCH':'PRESS SPACE TO LAUNCH');
        return;
      }
    }
  ...
  g.draw=gc=>{
    ...
    for(const b of bricks){
      gc.fillStyle=b.col;gc.fillRect(b.x,b.y,b.w,b.h);
    ...
    gc.textAlign='left';
    for(let i=0;i<3;i++){
      gc.beginPath();gc.arc(F.x+16+i*20,F.y+F.h+30,5,0,TAU);
      if(i<lives){gc.fillStyle=C.ball;gc.fill();}else{gc.strokeStyle=C.frame;gc.stroke();}
    }
    if(g.mode==='ready'){
      gc.fillStyle=(t%1<.5)?C.txt:'#5A5470';
...
/* ============================================================
   GAME: deep — hold-to-rise submersible, trench, mines, jellies
   ============================================================ */
Games.deep=(function(){
  const g={mode:'ready'};
  ...
  function reset(){
    g.mode='ready';dist=0;bonus=0;segs=[];mins=[];jels=[];bubs=[];
    sub.y=H*.5;sub.vy=0;spawnX=0;gap=.5;
    showHint(COARSE.matches?INFO.deep.hintC:INFO.deep.hintK);hideOver();
  }
  function start(){
    if(g.mode!=='ready')return;
    g.mode='playing';hideHint();hideOver();AudioFX.tone(120,300,.5,'sine',.35);
  }
  function die(){
    g.mode='dead';deadAt=performance.now();AudioFX.die();hideHint();
    const sc=Math.floor(dist/12)+bonus;
    const best=+LS.get(bestKey('deep'),0),nb=sc>best,mx=Math.max(best,sc);
    if(nb)LS.set(bestKey('deep'),sc);
    setBest(mx);
    setTimeout(()=>showOver(INFO.deep.over,sc,mx,nb),500);
  }
  g.enter=()=>{layout();reset();};
  g.start=start;
  g.restart=()=>{reset();start();};
  g.resize=layout;
  g.key=k=>{if(k===' '||k==='ArrowUp'||k==='w'||k==='W')thrust=true;};
  g.release=k=>{if(k===' '||k==='ArrowUp'||k==='w'||k==='W')thrust=false;};
  g.pointer=(e,type)=>{if(type==='down')thrust=true;else if(type!=='move')thrust=false;};
  g.update=dt=>{
    t+=dt;
  ...
  g.draw=gc=>{
    ...
    if(d<.7){
      gc.globalAlpha=(0.7-d)*.16;gc.fillStyle='#BFE8FF';
      for(let i=0;i<5;i++){
        const rx=W*(i*.22+.05)+Math.sin(t*.4+i)*30;
    ...
    for(const m of mins){
      const pu=1+Math.sin(t*4+m.ph)*.08;
    ...
    for(const j of jels){
      ...
      for(let i=-2;i<=2;i++){
        gc.beginPath();gc.moveTo(i*5*U,0);
